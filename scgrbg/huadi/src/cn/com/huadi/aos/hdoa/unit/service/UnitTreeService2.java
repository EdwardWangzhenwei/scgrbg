...
public class UnitTreeService2 {
	...
	public List<Map> getTreeData(DataMsgBus bus) {
		...
		String sql1="select code from (select t.unit_id code from priv_unit t where t.unit_flag=1 start with t.unit_id = 0 connect by t.unit_id = prior t.uper_unit_id order siblings by t.unit_sort) where rownum=1";
		Map<String,Object> map1=DbSvr.getDbService("acc_sys").getOneRecorder(sql1, null);
		if(map1!=null){
