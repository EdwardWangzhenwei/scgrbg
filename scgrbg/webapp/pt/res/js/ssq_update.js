...
function zwedit(wjbm,n_id){
	//window.open('pt/res/hdoa/wpsedit/edit.jsp?wjbm='+wjbm+'&n_id='+n_id);
	//以下方式为全屏打开弹出窗口
...
function openspd(wjbm,n_id){
	//window.open('pt/res/hdoa/wpsedit/spdview.jsp?wjbm='+wjbm+'&n_id='+n_id);
	//以下方式为全屏打开弹出窗口
...
function afterZwedit(tmp){
	//if(tmp){
		//PT.refresh();
	//}
	PT.showForm('ssq_update','wjbm='+wg('wjbm')+'&n_id='+wg('n_id'));	
}
function fjadd(wjbm){
	windowOpen('pt/res/hdoa/public/DzwjAdd.jsp?wjbm='+wjbm,'',750,300);
}
//新增页面上传附件，此时还没有wjbm，所以返回dzwj_id，保存文件时再修改dzwj记录的con_table_id
function fjadd2(){
	windowOpen('pt/res/hdoa/public/DzwjAdd.jsp','',750,300);
}
function afterFjadd(){
...
function createFjTab(res){
	var table=document.getElementById("fjlists.table");
	var tr,td;
	if(res&&res.length>0){
	  	for(var i=0;i<res.length;i++){
			...
			if(res[i].dzwjsx=='审批件'){				
				...
				td.innerHTML = res[i].dzwjsx+'    <a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>&nbsp;<input type=\"button\" class=\"gradual-btnitem\" style=\"width:60px;height:27px;margin-left:50px;\" value=\"删除\" onclick=\"deletefj('+res[i].dzwj_id+');\"/>';
			}
			tr.appendChild(td);
	  	}
...
function createFjTab2(res){
	var fjlists=document.getElementById("fjlists");alert(fjlists);
	fjlists.innerHTML = 'ddddddd<br>';
	fjlists.innerHTML =fjlists.innerHTML+ '22222222222<br>';
	...
	if(res&&res.length>0){
		for(var i=0;i<res.length;i++){
			...
			if(n_uperid&&n_uperid>0){
				td.innerHTML = '    <a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>';
			}else{				
				td.innerHTML = '    <a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>&nbsp;<input type=\"button\" class=\"gradual-btnitem\" style=\"width:60px;height:27px;margin-left:50px;\" value=\"删除\" onclick=\"deletefj('+res[i].dzwj_id+');\"/>';
			}
			tr.appendChild(td);
		}
		fjlist.appendChild(table);
...
//附件列表
function createFjTab2(res){
	var fjlists=document.getElementById("fjlists");
	...
	if(res&&res.length>0){
		...
		for(var i=0;i<res.length;i++){
			...
			//if((n_uperid&&n_uperid>0)||res[i].dzwjsx=='审批件'){
			if(n_uperid&&n_uperid>0){
				fjlists.innerHTML = fjlists.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a></span><br>';
			}else{				
				fjlists.innerHTML = fjlists.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>&nbsp;<input type=\"button\" class=\"gradual-btnitem\" style=\"border:0px;width:27px;height:27px;margin-left:50px;background:url(pt/res/js/img/delete_cron.png) no-repeat;cursor:pointer;\" onclick=\"deletefj2('+res[i].dzwj_id+');\"/></span><br>';
			}
		}
...
//附件列表（详情页用，无按钮）
function createFjTab5(res){
	var fjlists=document.getElementById("fjlists");
	fjlists.innerHTML='';
	if(res&&res.length>0){
		for(var i=0;i<res.length;i++){
			if(res[i].dzwjsx=='审批件'||res[i].dzwjsx=='正式件'){					
				fjlists.innerHTML = fjlists.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/show_wps.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a></span><br>';				
			}else{
				fjlists.innerHTML = fjlists.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a></span><br>';				
			}
		}
...
function fjadd3(){
	var res=PT.showModal('nowjbm_fj_add2','con_table=GW_FZXX&con_table_id='+wg('wjbm'),550,200,null);
	if(res==''){		
	}else if(!isNaN(res)){
...
function fjadd4(){
	var res=PT.showModal('nowjbm_fj_add2',null,550,200,null);
	if(res!=''&&!isNaN(res)){
		var dzwj_id=wg('dzwj_id');
		if(dzwj_id!=-1){
			w('dzwj_id').set(dzwj_id+=','+res);
...
function fjadd5(){
	var res=PT.showModal('nowjbm_fj_add2','con_table=GW_LDPS&con_table_id='+wg('text_ldps_id'),550,200,null);
	if(res==''){		
	}else if(!isNaN(res)){
