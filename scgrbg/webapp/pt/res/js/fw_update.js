...
function createFjTab1(res){
	...
	if(document.getElementById("detail")){
		btn='<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><button rawclass="gradual-btnitem" class="gradual-btnitem" id="fjadd2" title="上传附件" style="line-Height:27px;height:27px;width:70px;" _mc="PT.FlatBtn" "></button></span><br>';
	}else{
		btn='<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><button rawclass="gradual-btnitem" class="gradual-btnitem" id="fjadd2" title="上传附件" style="line-Height:27px;height:27px;width:70px;" _mc="PT.FlatBtn" onClick="fjadd();">上传附件</button></span><br>';
	}
	/*var btn='<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><button rawclass="gradual-btnitem" class="gradual-btnitem" id="fjadd2" title="上传附件" style="line-Height:27px;height:27px;width:70px;" _mc="PT.FlatBtn" onClick="fjadd();">上传附件</button></span><br>';*/
	...
	if(res&&res.length>0){
		...
		for(var i=0;i<res.length;i++){
				if(i==res.length-1){
					...
					if(document.getElementById("detail")){
						zsjs.innerHTML = zsjs.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:0px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>&nbsp;<input type=\"button\" class=\"gradual-btnitem\" style=\"border:0px;width:27px;height:27px;margin-left:50px;background:url() no-repeat;cursor:pointer;\" "/></span><br>';
					}else{
						zsjs.innerHTML = zsjs.innerHTML+'<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><a name=\"tm1\" href=\"pt/res/hdoa/wpsedit/ShowDzwj.jsp?dzwj_id='+res[i].dzwj_id+'\">'+res[i].dzwjm+'</a>&nbsp;<input type=\"button\" class=\"gradual-btnitem\" style=\"border:0px;width:27px;height:27px;margin-left:50px;background:url(pt/res/js/img/delete_cron.png) no-repeat;cursor:pointer;\" onclick=\"deletefj(\''+res[i].dzwj_id+'\');\"/></span><br>';
					}
		}
...
function createFjTab(res){
	var zsjs=document.getElementById("zsjs");
	var btn='<span id="btn_span" class="bsw-c" style="height:27px;width:auto;margin-left:25px;"><button rawclass="gradual-btnitem" class="gradual-btnitem" id="fjadd2" title="上传附件" style="line-Height:27px;height:27px;width:70px;" _mc="PT.FlatBtn" onClick="fjadd1();">上传附件</button></span><br>';
	//每次初始化时只保留“上传附件”按钮（注：只针对起始节点，其他节点没有上传操作，没有“上传附件”按钮时，显示“批量下载”按钮，将文件打包下载）
	zsjs.innerHTML=btn;
	//发文整个过程都显示上传按钮
	//nzfjlists.innerHTML=btn;
...
//接待附件新增
function fjadd(){
	var res=PT.showModal('fj','union_table=jd_jdxx&union_table_id='+wg('jdxx_id'),550,250,null);
	if(res!=null){
		w('jdxx_id').set(res);
