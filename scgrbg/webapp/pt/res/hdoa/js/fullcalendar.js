...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			if (timeText) {
				timeHtml = '<span class="fc-time" style="cursor:pointer">' + htmlEscape(timeText) + '</span>';
			}
		}
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	triggerDayDbClick: function(span, dayEl, ev) {
		this.trigger(
			'dayDbClick',
			dayEl,
...
function Calendar_constructor(element, overrides) {
	...
	t.getNow = function() {
		...
		var now = t.moment(defaultDate1).stripZone();
...
function DaysNumberofDate(DateGL) {
    return parseInt((Date.parse(DateGL.year()+"/"+DateGL.month()+"/"+DateGL.date()) - Date.parse(DateGL.year() + "/1/1")) / 86400000) + 1; 
} 
function CnDateofDate(DateGL) {
    ...
    var ResultMonth; 
    var ResultDay; 
    var yyyy = DateGL.year(); 
    var mm = DateGL.month() + 1; 
    var dd = DateGL.date();
...
function CnDayofDate(DateGL) { 
    var CnDayStr = new Array("零", 
        ...
        "廿六", "廿七", "廿八", "廿九", "三十"); 
    var Day; 
    Day = (Math.abs(CnDateofDate(DateGL))) % 100; //abs求绝对值
    alert(Day);
    //hanlichen mod 
    if ("初一" == CnDayStr[Day]) { 
...
function SolarTerm(DateGL) {
    ...
    for (; DateGL.year() < BeginTime.getFullYear();) { 
        BeginTime.setTime(BeginTime.getTime() - DifferenceInYear * 1000); 
    } 
    for (; DateGL.year() > BeginTime.getFullYear();) { 
        BeginTime.setTime(BeginTime.getTime() + DifferenceInYear * 1000); 
    } 
    for (var M = 0; DateGL.month() > BeginTime.getMonth(); M++) { 
        BeginTime.setTime(BeginTime.getTime() + DifferenceInMonth[M] * 1000); 
    } 
    if (DateGL.date() > BeginTime.getDate()) { 
        BeginTime.setTime(BeginTime.getTime() + DifferenceInMonth[M] * 1000); 
        M++; 
    } 
    if (DateGL.date() > BeginTime.getDate()) { 
