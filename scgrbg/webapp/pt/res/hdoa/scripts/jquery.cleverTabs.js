...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于显示的Panel模板
        tabPanelTemplate: '<div id="cleverTabPanelItem-#{id}" style="border: 1px solid  #a99c7e;  margin:0 auto; width:100%; margin: 0px;float: left;padding-top: 0px !important;"><iframe frameBorder="0" style="width: 100%; display: inline;height:520px" scrolling="auto" src="#{src}"></iframe></div>',
        //Tab唯一id生成器
...
//获取指定Url的Tab
CleverTabs.prototype.getSameTabByUrl = function (url) {
    if (!url) {
        return null;
    }
    var self = this;
    ...
    for (i = 0; i < frames.size(); i++) {
        ...
        //	if (src.indexOf(url.toLowerCase())>=0) {
            tab = new CleverTab(self, CleverTab.getUniqueByPanelId(frame.parent('div:first').attr('id')));
        }
    }
    return tab;
...
CleverTab.prototype.activate = function () {
	...
	var newUrl = self.panel.find(' > iframe:first').attr('src');
    if(newUrl.indexOf("home")>0){
		var frame = self.panel.find(' > iframe:first');
		var contentDocument = frame[0].contentWindow;
		var filelist= contentDocument.document.getElementById('filelist');
		if(filelist!=null){
			if(filelist.rows.length<=1){
				self.refresh();
			}
		}
	}
...
//刷新该Tab的iframe中的内容
CleverTab.prototype.load = function (url) {
    var self = this;
    var frame = self.panel.find(' > iframe:first');   
    frame.attr('src', url);
}
